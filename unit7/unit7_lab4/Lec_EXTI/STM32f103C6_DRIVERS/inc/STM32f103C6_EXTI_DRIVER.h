...
//-----------------------------
typedef struct {
	uint16_t EXTI_INPUT_LineNumber; 		// This parameter must be set based on @ref EXTI_LINES_define
	GPIO_TypeDef *GPIO_PORT;
	uint16_t GPIO_PIN;						// This parameter must be set based on @ref GPIO_PINS_define
	uint8_t IVT_IRQ_NUMER;
}EXTI_GPIO_Mapping_t;
...
typedef struct {
	...
	uint8_t				IRQ_EN;				// Enable or Disable the EXTI IRQ (That will enable the IRQ mask in EXTI and NVIC controller)
											// This parameter must be set based on @ref EXTI_IRQ_define

	void (*P_IRQ_CALLBACK_FUN)(void);			// Set the C Function() which will be called once the IRQ happen
}EXTI_PinConfig_t;
...
#define AFIO_GPIO_EXTI_Selector(x)   ( (x==GPIOA)?0:\
		(x==GPIOB)?1:\
				(x==GPIOC)?2:3)
/*
* ===============================================
